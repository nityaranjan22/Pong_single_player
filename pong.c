...
//Made the ball and paddle faster for more fun
//Up is w
//Down is s
//use l to make the paddle larger
//use k to make the paddle smaller
...
int main(void) {
    ...
    while (!quit) {
        ...
        switch (getch()) {
            case 'w':
                if (paddle.py - 1 > 1) paddle.py -= 2;
                break;

            case 's':
                if ((paddle.py + paddle.length) + 1 < max_y) paddle.py += 2;
                break;
